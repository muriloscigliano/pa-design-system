...
import { PaTable } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('patable')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaTable } from './components'
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`patable-states-${activeStatesTab}`)"
        >
          {{ showCode[`patable-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`patable-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`patable-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaTable component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('patable')"
        copy-key="patable-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaTable component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="patable-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
