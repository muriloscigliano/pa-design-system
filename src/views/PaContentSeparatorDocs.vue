...
import { PaContentSeparator } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('pacontentseparator')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaContentSeparator } from './components'
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`pacontentseparator-states-${activeStatesTab}`)"
        >
          {{ showCode[`pacontentseparator-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`pacontentseparator-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`pacontentseparator-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaContentSeparator component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('pacontentseparator')"
        copy-key="pacontentseparator-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaContentSeparator component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="pacontentseparator-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
