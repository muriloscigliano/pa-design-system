...
import { PaPagination } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('papagination')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaPagination } from './components'

<PaPagination :model-value="1" :total="100" />`
  }
  return codes[tab] || ''
}
</script>
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`papagination-states-${activeStatesTab}`)"
        >
          {{ showCode[`papagination-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`papagination-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`papagination-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaPagination component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('papagination')"
        copy-key="papagination-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaPagination component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="papagination-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
