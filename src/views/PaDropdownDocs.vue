...
import { PaDropdown } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('padropdown')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaDropdown } from './components'
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`padropdown-states-${activeStatesTab}`)"
        >
          {{ showCode[`padropdown-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`padropdown-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`padropdown-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaDropdown component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('padropdown')"
        copy-key="padropdown-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaDropdown component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="padropdown-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
