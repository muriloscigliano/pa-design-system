...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('paicon')
})

const getUsageCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaIcon } from './components'
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode('paicon-usage')"
        >
          {{ showCode['paicon-usage'] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode['paicon-usage']" class="code-preview">
          <CodeBlock 
            :code="getUsageCode(activeTab)"
            :copy-key="'paicon-usage-code'"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaIcon component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('paicon')"
        copy-key="paicon-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaIcon component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="paicon-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
