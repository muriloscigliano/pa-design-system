...
import { PaNavButton } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('panavbutton')
})

const getUsageCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaNavButton } from './components'
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode('panavbutton-usage')"
        >
          {{ showCode['panavbutton-usage'] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode['panavbutton-usage']" class="code-preview">
          <CodeBlock 
            :code="getUsageCode(activeTab)"
            :copy-key="'panavbutton-usage-code'"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaNavButton component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('panavbutton')"
        copy-key="panavbutton-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaNavButton component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="panavbutton-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
