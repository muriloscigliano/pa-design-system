...
import { PaRangeSlider } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('parangeslider')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaRangeSlider } from './components'

<PaRangeSlider :model-value="[20, 80]" />`
  }
  return codes[tab] || ''
}
</script>
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`parangeslider-states-${activeStatesTab}`)"
        >
          {{ showCode[`parangeslider-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`parangeslider-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`parangeslider-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaRangeSlider component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('parangeslider')"
        copy-key="parangeslider-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaRangeSlider component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="parangeslider-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
