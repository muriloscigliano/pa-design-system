...
import { PaAutocomplete } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('paautocomplete')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaAutocomplete } from './components'

<PaAutocomplete :options="[]" />`
  }
  return codes[tab] || ''
}
</script>
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`paautocomplete-states-${activeStatesTab}`)"
        >
          {{ showCode[`paautocomplete-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`paautocomplete-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`paautocomplete-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaAutocomplete component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('paautocomplete')"
        copy-key="paautocomplete-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaAutocomplete component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="paautocomplete-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
