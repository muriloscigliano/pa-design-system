...
import { PaEmptyState } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('paemptystate')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaEmptyState } from './components'
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`paemptystate-states-${activeStatesTab}`)"
        >
          {{ showCode[`paemptystate-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`paemptystate-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`paemptystate-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaEmptyState component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('paemptystate')"
        copy-key="paemptystate-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaEmptyState component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="paemptystate-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
