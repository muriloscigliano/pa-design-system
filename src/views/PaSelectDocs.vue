...
import { PaSelect } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('paselect')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaSelect } from './components'

<PaSelect :options="[{ label: 'Option 1', value: 1 }, { label: 'Option 2', value: 2 }]" />`
  }
  return codes[tab] || ''
}
</script>
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`paselect-states-${activeStatesTab}`)"
        >
          {{ showCode[`paselect-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`paselect-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`paselect-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaSelect component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('paselect')"
        copy-key="paselect-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaSelect component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="paselect-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
