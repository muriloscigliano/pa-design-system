...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('patabs')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaTabs } from './components'

<PaTabs :model-value="1" />`
  }
  return codes[tab] || ''
}
</script>
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`patabs-states-${activeStatesTab}`)"
        >
          {{ showCode[`patabs-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`patabs-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`patabs-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaTabs component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('patabs')"
        copy-key="patabs-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaTabs component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="patabs-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
