...
import { PaTimePicker } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('patimepicker')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaTimePicker } from './components'
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`patimepicker-states-${activeStatesTab}`)"
        >
          {{ showCode[`patimepicker-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`patimepicker-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`patimepicker-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaTimePicker component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('patimepicker')"
        copy-key="patimepicker-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaTimePicker component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="patimepicker-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
