...
import { PaProgress } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('paprogress')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaProgress } from './components'
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`paprogress-states-${activeStatesTab}`)"
        >
          {{ showCode[`paprogress-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`paprogress-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`paprogress-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaProgress component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('paprogress')"
        copy-key="paprogress-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaProgress component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="paprogress-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
