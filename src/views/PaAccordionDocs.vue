...
import { PaAccordion } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('paaccordion')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaAccordion } from './components'
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`paaccordion-states-${activeStatesTab}`)"
        >
          {{ showCode[`paaccordion-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`paaccordion-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`paaccordion-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaAccordion component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('paaccordion')"
        copy-key="paaccordion-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaAccordion component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="paaccordion-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
