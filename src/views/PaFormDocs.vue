...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('paform')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaForm } from './components'
  ...
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        Form wrapper component for managing form state and validation.
      </p>
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`paform-states-${activeStatesTab}`)"
        >
          {{ showCode[`paform-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`paform-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`paform-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaForm component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('paform')"
        copy-key="paform-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaForm component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="paform-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
