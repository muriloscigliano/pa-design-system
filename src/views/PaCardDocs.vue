...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('pacard')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaCard } from './components'
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`pacard-states-${activeStatesTab}`)"
        >
          {{ showCode[`pacard-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`pacard-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`pacard-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaCard component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('pacard')"
        copy-key="pacard-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaCard component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="pacard-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
