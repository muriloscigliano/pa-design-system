...
import { PaContainer } from '../components'
...
onMounted(async () => {
  tokenSource.value = await getComponentTokens('pacontainer')
})

const getStatesCode = (tab: string): string => {
  const codes: Record<string, string> = {
    'default': `import { PaContainer } from './components'
...
<template>
  <div class="component-docs">
    <div class="documentation-section">
      ...
      <div class="example-preview-container">
        ...
        <button 
          ...
          @click="toggleShowCode(`pacontainer-states-${activeStatesTab}`)"
        >
          {{ showCode[`pacontainer-states-${activeStatesTab}`] ? 'Hide code' : 'Show code' }}
        </button>
        <div v-if="showCode[`pacontainer-states-${activeStatesTab}`]" class="code-preview">
          <CodeBlock 
            :code="getStatesCode(activeStatesTab)"
            :copy-key="`pacontainer-states-usage-${activeStatesTab}`"
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        The complete source code for the PaContainer component.
      </p>
      <CodeBlock 
        :code="getComponentSourceCode('pacontainer')"
        copy-key="pacontainer-component-source"
        :show-line-numbers="true"
        language="vue"
      />
    </div>
    ...
    <div class="documentation-section">
      ...
      <p class="documentation-section-description">
        CSS custom properties (design tokens) used by the PaContainer component.
      </p>
      <CodeBlock 
        :code="tokenSource"
        copy-key="pacontainer-token-source"
        :show-line-numbers="true"
        language="css"
      />
    </div>
